...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderProfile() {
    ...
    if (profileMetaEl) {
      ...
      profileMetaEl.textContent = `${followersCount} подписчиков · ${followingCount} подписок · Регистрация: ${new Date(
        user.createdAt
    ...
    if (profileComposerSection) {
      const isOwn = current && current.id === user.id;
      profileComposerSection.style.display = isOwn ? "flex" : "none";
      if (profileEditBtn) {
      ...
      if (profileFollowBtn) {
        if (!current || isOwn) {
          ...
          profileFollowBtn.style.display = "inline-flex";
    ...
    if (!posts.length) {
      profilePostsEl.innerHTML = `
        <section class="empty-state">
          ...
          <p class="empty-title">Нет постов</p>
          <p class="empty-text">
            ${current && current.id === user.id ? "Поделитесь своим первым постом в Социальной Сети Zachetka." : "У пользователя пока нет постов."}
          </p>
        </section>
      `;
      return;
    ...
    posts.forEach((post) => {
      ...
      card.innerHTML = `
        <header class="post-header">
          <div class="post-avatar js-profile-link" data-user-id="${user.id}">
            ${user.avatarUrl ? `<img src="${user.avatarUrl}" alt="" />` : user.displayName[0]?.toUpperCase() || "U"}
          </div>
          <div>
            <div class="post-author js-profile-link" data-user-id="${user.id}">
              ${user.displayName}
            </div>
            <div class="post-meta">${timeAgo(post.createdAt)} • @${user.username}</div>
          </div>
        </header>
        <div class="post-content">
          <p class="post-text">${post.text}</p>
      ...
      profilePostsEl.appendChild(card);
    });
  }
